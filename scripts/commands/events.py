...
@click.option("--description", prompt="Event description (English)", help="Event description in English")
@click.option("--description-fr", help="Event description in French (optional)")
@click.option("--start-date", prompt="Start date (YYYY-MM-DD HH:MM)", help="Start date and time")
...
@click.option("--event-type", type=click.Choice(["competition", "meeting", "conference", "training", "workshop"]),
              prompt="Event type", help="Type of event")
@click.option("--url", help="Optional external URL")
@click.option("--registration-deadline", help="Registration deadline (YYYY-MM-DD HH:MM, optional)")
def create_event(title, title_fr, description, description_fr, start_date, start_timezone,
                end_date, end_timezone, location, event_type, url, registration_deadline):
...
def list_events(format, upcoming):
    ...
    if upcoming:
        now = datetime.now()
        events_list = [e for e in events_list if datetime.fromisoformat(e.get('start_date', '').replace('Z', '+00:00')) > now]
        ...
        for i, event in enumerate(events_list, 1):
            ...
            if isinstance(title, dict) and isinstance(description, dict):
                if ('en' in title) != ('en' in description):
                    warnings.append(f"Event {i}: Inconsistent language fields between title and description")
...
@click.option("--dry-run", is_flag=True, help="Preview what would be pruned without making changes")
...
def prune_events(dry_run, before):
    ...
    try:
        ...
        console.print(f"[yellow]Found {len(events_to_remove)} past events to prune:[/yellow]")
        ...
        if dry_run:
            console.print(f"[blue]Dry run complete - would remove {len(events_to_remove)} events[/blue]")
            return
