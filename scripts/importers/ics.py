...
class ICSImporter(BaseImporter):
    def add_arguments(self, parser):
        parser.add_argument("-i", "--input", help="Input ICS file path or URL", required=True)
        parser.add_argument("-o", "--output", help="Output events.toml file",
                          default="content/data/events.toml")
        parser.add_argument("--dry-run", help="Preview events without writing to file",
                          action="store_true")
        parser.add_argument("--event-type", help="Event type to assign", default="event")
            ...
            if component.name == "VEVENT":
                ...
                if 'DTSTART' in component:
                    ...
                    if isinstance(dtstart, datetime):
                        ...
                        event['start_date'] = datetime.combine(dtstart, datetime.min.time()).isoformat()
                ...
                if 'DTEND' in component:
                    ...
                    if isinstance(dtend, datetime):
                        ...
                        event['end_date'] = datetime.combine(dtend, datetime.min.time()).isoformat()
    ...
    def is_duplicate_event(self, new_event, existing_event):
        ...
        if isinstance(new_title, dict) and isinstance(existing_title, dict):
            title_match = new_title.get('en', '').strip() == existing_title.get('en', '').strip()
        ...
        start_match = new_event.get('start_date', '') == existing_event.get('start_date', '')
        location_match = new_event.get('location', '').strip() == existing_event.get('location', '').strip()
    ...
    def run(self, args):
        try:
            ...
            print(f"[green]Successfully added {len(new_events)} new events to {output_path}[/green]")
